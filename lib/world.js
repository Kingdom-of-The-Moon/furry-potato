...
module.exports.World = class World extends EventEmitter {
    constructor(chunkGen, size) {
        super();
    ...
    loadChunk(chunk) {
        ...
        if (fs.existsSync(`./chunks/${chunk.x}_${chunk.y}.json`)) {
            let Chunk = require(`../chunks/${chunk.x}_${chunk.y}.json`);
            this.chunks[`${chunk.x}_${chunk.y}`] = Chunk;
        } else {
            let Chunk = this.chunkGen(chunk.x, chunk.z);
            Chunk.x = chunk.x;
            Chunk.z = chunk.z;
            this.chunks[`${chunk.x}_${chunk.y}`] = Chunk;
            console.log(Chunk);
